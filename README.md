...
* **Optional Cloud Sync** – Microsoft Authentication Library (MSAL) connects to Microsoft 365; when signed in, the app now performs continuous two-way synchronization with SharePoint Lists through the `sharepoint.ts` service. Configuration lives in `constants.ts`.
