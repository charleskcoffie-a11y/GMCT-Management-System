...
describe('resolveTableTarget', () => {
    ...
    it('supports strings wrapped in single quotes', () => {
        const result = resolveTableTarget('\'public.entries\'', defaultTable, 'entries');
        expect(result).toEqual({ path: 'entries', schema: 'public' });
    });

    it('supports individually single quoted schema and table segments', () => {
        const result = resolveTableTarget('\'finance archive\'.\'Entries Table\'', defaultTable, 'entries');
        expect(result).toEqual({ path: 'Entries%20Table', schema: 'finance archive' });
    });
