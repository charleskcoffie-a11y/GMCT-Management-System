...
const AdminAttendanceView: React.FC<AdminAttendanceViewProps> = ({ members, attendance, settings, currentUser }) => {
    ...
    const classGroups = useMemo(() => {
        ...
        const groups = new Map<string, { label: string; key: string; members: Array<{ member: Member | undefined; status: AttendanceStatus }>; counts: Record<AttendanceStatus, number> }>();
    ...
    const filteredClassGroups = useMemo(() => {
        ...
        return classGroups
            .filter(group => classFilter === 'all' || group.key === classFilter)
            .map(group => {
    ...
    }, [classGroups, memberFilter, classFilter]);
            ...
            <section className="rounded-3xl shadow-lg border border-white/60 bg-gradient-to-br from-white via-sky-50 to-cyan-100/70 p-6 space-y-4">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:items-center">
                        ...
                        />
                        <select value={classFilter} onChange={e => setClassFilter(e.target.value as typeof classFilter)} className="border border-slate-300 rounded-lg px-3 py-2">
                            <option value="all">Show all classes</option>
                            {Array.from({ length: settings.maxClasses }, (_, index) => String(index + 1)).map(option => (
